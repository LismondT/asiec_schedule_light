...
class AsiecScheduleRemoteDatasource extends ScheduleRemoteDatasource {
  ...
  Map<String, String> _getData(DateTime startDate, DateTime endDate, ScheduleRequestType type, String id) {
    return {
      'dostup': 'true',
      switch (type) {
  ...
  List<DayEntity> parseSchedule(String html) {
    ...
    List<LessonEntity> currentLessons = [];

    for (final row in rows) {
      if (row.querySelector('.den') != null) {
        // Это строка с датой
        if (currentDate != null && currentLessons.isNotEmpty) {
          days.add(DayEntity(date: currentDate, lessons: currentLessons));
        }
        currentLessons = [];
        ...
        final number = numberMatch != null ? int.parse(numberMatch.group(1)!) : 0;

        final timeMatch = RegExp(r'(\d{1,2}:\d{2})\s*-\s*(\d{1,2}:\d{2})').firstMatch(numberText);
        TimeOfDay? startTime;
        TimeOfDay? endTime;
        ...
        if (name.isNotEmpty && startTime != null && endTime != null) {
          currentLessons.add(LessonEntity(
            number: number,
            name: name,
            group: group,
            subgroup: subgroup,
            teacher: teacher.isNotEmpty ? teacher : 'Не указан',
            classroom: classroom.isNotEmpty && classroom != '"' ? classroom : 'не указана',
